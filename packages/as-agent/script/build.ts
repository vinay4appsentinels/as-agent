...
import { Script } from "@as-agent/script"
...
// Dependencies already installed at workspace level
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/as-agent`,
      execArgv: [`--user-agent=as-agent/${Script.version}`, `--env-file=""`, `--`],
      windows: {},
    },
    entrypoints: ["./src/index.ts", parserWorker, workerPath],
    define: {
      ASAGENT_VERSION: `'${Script.version}'`,
      OTUI_TREE_SITTER_WORKER_PATH: "/$bunfs/root/" + path.relative(dir, parserWorker).replaceAll("\\", "/"),
      ASAGENT_WORKER_PATH: workerPath,
      ASAGENT_CHANNEL: `'${Script.channel}'`,
    },
  })
